...
function createThunkMiddleware<S, A>(extraArgument?: any) {
  return ({ dispatch, getState }: {dispatch: (a: A) => void, getState: () => S}) => (next: (a: A) => void) => (action: A) => {
...
function useEnhancedReducer<S, A>(reducer: (s: S, a: A) => S, initialState: S, middlewares = [] as any): [S, (a: A) => void] {
